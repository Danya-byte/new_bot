...
class UserStates(StatesGroup):
    remove_burger_id = State()
    remove_quantity = State()
    quantity = State()
...
async def burger_details(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    if burger:
        ...
        quantity = (await state.get_data()).get('quantity', 1)
...
async def increase_quantity(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    await update_quantity(callback_query, burger_id, quantity, state)
...
async def update_quantity(callback_query: types.CallbackQuery, burger_id: int, quantity: int, state: FSMContext):
    await bot.answer_callback_query(callback_query.id)
...
async def confirm_remove_from_cart(callback_query: types.CallbackQuery, state: FSMContext):
    await bot.answer_callback_query(callback_query.id)
    ...
    if len(data_parts) == 4 and data_parts[0] == 'remove' and data_parts[1] == 'from' and data_parts[2] == 'cart' and \
        ...
        if burger:
            ...
            await state.update_data(remove_burger_id=burger_id, remove_quantity=quantity)
        else:
...
async def handle_remove_quantity(message: types.Message, state: FSMContext):
    user_input = message.text
    try:
        quantity_to_remove = int(user_input)
        user_id = message.from_user.id
        current_state = await state.get_data()
        burger_id = current_state.get('remove_burger_id')
        max_quantity = current_state.get('remove_quantity')
...
async def admin_remove_burger(message: types.Message, state: FSMContext):
    user_id = message.from_user.id
    ...
    await state.set_state(UserStates.remove_burger_id)

@dp.message(UserStates.remove_burger_id)
async def handle_burger_id(message: types.Message, state: FSMContext):
    user_id = message.from_user.id
    if user_id != config.ADMIN_USER_ID:
        await message.reply('У вас нет прав для выполнения этой команды.')
        return
